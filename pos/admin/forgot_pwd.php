...
if (isset($_POST['reset_pwd'])) {
  ...
  $checkEmail = mysqli_query($mysqli, "SELECT `email_admin` FROM `admin` WHERE `email_admin` = '" . $_POST['reset_email'] . "'") or exit(mysqli_error($mysqli));
  if (mysqli_num_rows($checkEmail) > 0) {
    ...
    $reset->execute();
    if ($reset) {
      $success = "Mot de passe Reset Instructions Sent To Your Email";
      // && header("refresh:1; url=index.php");
    } else {
      $err = "Veuillez réessayer";
    }
  } else {
    $err = "Aucun compte avec ce mail";
  }
}
      ...
      <div class="container mt--8 pb-5">
        <div class="row justify-content-center">
          <div class="col-lg-5 col-md-7">
              ...
              <div class="card-body px-lg-5 py-lg-5">
                <form method="post" role="form">
                  ...
                  <div class="text-center">
                    <button type="submit" name="reset_pwd" class="btn btn-primary my-4">Réinitialiser mot de
                      passe</button>
                  </div>
                </form>
              </div>
            ...
            <div class="row mt-3">
              <div class="col-6">
                <a href="index.php" class="text-light"><small>Se connecter?</small></a>
              </div>
            </div>
          </div>
        </div>
