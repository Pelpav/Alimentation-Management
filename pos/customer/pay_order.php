...
if (isset($_POST['pay'])) {
    ...
    if (empty($_POST["code_paiements"]) || empty($_POST["montant_paiements"]) || empty($_POST['methode_paiements'])) {
        ...
        if (strlen($code_paiements) < 10) {
            $err = "Code de Paiement Verification Failed, Please Try Again";
        } elseif (strlen($code_paiements) > 10) {
            $err = "Code de Paiement Verification Failed, Please Try Again";
        } else {
            $code_paiements = $_POST['code_paiements'];
                                                    ...
                                                    <div class="card-body">
                                                        <form method="POST" enctype="multipart/form-data">
                                                            <div class="form-row">
                                                                ...
                                                                <div class="col-md-6">
                                                                    <label>Code de Paiement</label><small class="text-danger"> 10 Charactères
                                                                        Alpha-numérique Si la méthode de paiement est le CASH</small>
                                                                    <input type="text" limit="11" name="code_paiements"
                                                                        placeholder="<?php echo $mpesaCode; ?>" class="form-control" value="">
                                                                </div>
                                                            </div>
                                                            <hr>
                                                            <div class="form-row">
